...
import { dijkstras, tracePath } from './Dijkstra';
...
class Pathfinding extends Component {
	...
	// DISABLES LINKS WHILE ANIMATING
	disableLinks() {
	...
	handleMouseDown(node, row, col) {
		...
		if (node.isStart) {
			let newGrid = getNewStart(this.state.grid, row, col);
			this.setState({ changingStart: true, grid: newGrid, clicked: true });
			return;
		} else if (node.isFinish) {
			let newGrid = getNewFinish(this.state.grid, row, col);
			this.setState({ changingFinish: true, grid: newGrid, clicked: true });
			return;
		} else {
			let newGrid = getNewGridWithWalls(this.state.grid, row, col);
